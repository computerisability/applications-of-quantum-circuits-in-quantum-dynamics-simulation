...
h_xyz = xyz_hamiltonian(n_qubits, [0.25, 0.25, 0.25, -1])
h_drive = floquet_term(n_qubits)
coeff = np.sin(  0 * times)
...
up = basis(2,0)
down = basis(2,1)
psi0_tensor = tensor([down, up, down, up, down, up, down, up])         #修改初态

#psi0_tensor = tensor([down, up]) 
#print(psi0_tensor)
...
output = mesolve(h, psi0, times, e_ops=observables, options=options, args={'w':0})
#output1 = mesolve(h, psi0, times)

x0, z0, x0x1, z0z1 = output.expect[0:4]                                                  



psi_t = output.states
states_data = [state.full() for state in psi_t]  # 转为 NumPy 数组
#with open('driven_exact_states.pkl', 'wb') as f:                         #exact用pkl类文件是可以的
#    pickle.dump(psi_t, f)
...
for psi in psi_t:
    ...
    reduced_psi = psi0_tensor.ptrace(list(range(n_qubits//2)))
...
#fig 5:
loschmidt_echo = [-np.log(abs(psi0.overlap(psi))**2)/n_qubits for psi in psi_t]
...
#np.savez('driven_exact.npz', times=times, x0 = x0, z0 = z0, x0x1 = x0x1, z0z1 = z0z1,
