...
## Hamiltonian in file:///C:/Users/lidaming/Downloads/q-2024-02-13-1252.pdf
#def ising_special(n_qubits, A, B):
#	h = 0  
#	for k in range(n_qubits):
#		h -= A * one_site_op(n_qubits, k, X)
#        w1 = 1
#        w2 = 0.5
#	for k in range(n_qubits-1):
#                if k % 2 == 1:
#		   h -= w1 * nearest_neigh_op(n_qubits, k, [Z, Z])  
#                else:
#		   h -= w2 * nearest_neigh_op(n_qubits, k, [Z, Z])  
#	return h
...
def xyz_hamiltonian(n_qubits, model_params, bound_cond='open'):
	...
	jxx, jyy, jzz, jx, jy, jz = model_params  # unpack the model parameters
	h = 0  # initialize the Hamiltonian

	# X,Y,Z terms
	for k in range(n_qubits):
		h += jx * one_site_op(n_qubits, k, X)
		h += jy * one_site_op(n_qubits, k, Y)
		h += jz * one_site_op(n_qubits, k, Z)

	# If the boundary conditions are periodic, set n_max = n_qubits
	n_max = n_qubits - 1 if bound_cond == 'open' else n_qubits
	for k in range(n_max):
		h += jxx * nearest_neigh_op(n_qubits, k, [X, X])  # XX terms
		h += jyy * nearest_neigh_op(n_qubits, k, [Y, Y])  # YY terms
		h += jzz * nearest_neigh_op(n_qubits, k, [Z, Z])  # ZZ terms
...
#def ising_floquet_hamiltonian(n_qubits, model_params, time=None, bound_cond='open'):
def ising_floquet_hamiltonian(n_qubits, model_params, bound_cond='open'):
	""" Combine the XYZ Hamiltonian with a Floquet (term to make the system non-integrable)
	...
	w = 1  # frequency of oscillation of the Floquet term
	h = xyz_hamiltonian(n_qubits, model_params, bound_cond=bound_cond)
	#h += 0 * floquet_term(n_qubits) * np.sin(w * time)
	return h.reduce()
...
def hubbard_ladder_hamiltonian(size_x, model_params):
...
#def schwinger_hamiltonian(n_qubits, model_params, time = None):
def schwinger_hamiltonian(n_qubits, model_params):
    a, g, m, ell = model_params
