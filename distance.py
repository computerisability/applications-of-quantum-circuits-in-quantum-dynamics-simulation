...
with open('hubbard_exact_states.pkl', 'rb') as f:
    exact_states = pickle.load(f)             #是一个列表，每个元素均为向量
exact_states = exact_states[1:41]               #不要第一个元素

with open('hubbard_trotter.qpy', 'rb') as f:           #TROTTER做出来，缺少第一个元素
...
with open('hubbard_pvqd.qpy', 'rb') as f:           #TROTTER做出来，缺少第一个元素
    pvqd_states = qpy.load(f)
pvqd_states = pvqd_states[1:41]

#with open('hubbard_local.qpy', 'rb') as f:           #TROTTER做出来，缺少第一个元素
#    local_states = qpy.load(f)
#local_states = local_states[1:]

with open('hubbard_linear.qpy', 'rb') as f:           #TROTTER做出来，缺少第一个元素
    nonlocal_states = qpy.load(f)
#nonlocal_states = nonlocal_states[1:41]
...
local_states_qobj = [
    Qobj(statevector.data.reshape(-1, 1), dims=[[16], [1]])                       #这里手工修改dim
...
ax.errorbar(times, nonlocal_BURES, label="Trotter linear", marker='o', linestyle='', elinewidth=1, color='red', capsize=2, markersize=3.5)
