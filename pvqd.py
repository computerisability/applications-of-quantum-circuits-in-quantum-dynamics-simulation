...
ansatz = AnsatzXYZFloquet(n_qubits, depth=3)                    #原始代码此处为3，而adaptive这里其实是0
     
initial_parameters = np.zeros(ansatz.circuit.num_parameters)

hamiltonian = partial(xyz_floquet_hamiltonian, n_qubits=n_qubits, model_params=[0.25, 0.25, 0.25, 1])
print(hamiltonian)
...
np.savez('driven_pvqd.npz', x0 = x0, z0 = z0,    x0x1 = x0x1, z0z1 = z0z1,
...
with open('driven_pvqd.qpy', 'wb') as f:                              #量子电路算出来，需要保存为qpy文件
    qpy.dump(a, f)  
