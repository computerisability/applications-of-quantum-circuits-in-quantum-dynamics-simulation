...
with open('driven_exact_states.pkl', 'rb') as f:
    exact_states = pickle.load(f)             #是一个列表，每个元素均为向量
exact_states = exact_states[1:]               #不要第一个元素

with open('driven_pvqd.qpy', 'rb') as f:           #TROTTER做出来，缺少第一个元素
    trotter_states = qpy.load(f)
trotter_states = trotter_states[1:]           #只有TROTTER,需要注释掉这句
#print(approximate_states[2].draw())           #draw可将量子电路画出，且显示参数

with open('driven_linear.qpy', 'rb') as f:           #TROTTER做出来，缺少第一个元素
    pvqd_states = qpy.load(f)
#pvqd_states = pvqd_states[1:]

with open('driven_square.qpy', 'rb') as f:           #TROTTER做出来，缺少第一个元素
    local_states = qpy.load(f)
#local_states = local_states[1:]

with open('driven_nonlocal.qpy', 'rb') as f:           #TROTTER做出来，缺少第一个元素
...
ax.plot(times, trotter_BURES, label="p-VQD n=3", linestyle='dashed', linewidth=1, color='black')
ax.errorbar(times, pvqd_BURES, label="Trotter linear", marker='o', linestyle='', elinewidth=1, color='red', capsize=2, markersize=3.5)
ax.errorbar(times, local_BURES, label="Trotter square", marker='^', linestyle='', elinewidth=1, color='blue', capsize=2, markersize=3.5)
