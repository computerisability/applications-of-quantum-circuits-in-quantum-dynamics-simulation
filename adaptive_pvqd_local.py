...
class AdaptivePVQD:
    ...
    def get_loss(
        ...
        def infidelity(shift_values: np.array):
            ...
            #print('below is fidelity')
            #print(np.real(sampled_overlap.eval()))
    ...
    def adaptive_step(
        ...
        operator_pool = self.ansatz.create_op_pool_local(['x','y','z'], ['xx','yy','zz'])
    ...
    def minimization_routine(
        ...
        #这一步得到的infidelity,gradient，不是一个数！！！！而是一个函数！！！！
        
        updated_shift = np.copy(shift_init_guess)
        fid = 1 - infidelity(updated_shift)
        infid = infidelity(updated_shift)
        #print(f'the infidelity is {infid}')
        #print(f'the fidelity is {fid}')
        grad_norm = max(np.abs(gradient(updated_shift)))  # infinite norm
        #print(f'the grad norm is {grad_norm}')
        ...
        while grad_norm > self.gradient_tolerance and count < self.maxiter:  
            count += 1
            #print(f'this is the {count} optimization')
    ...
    def evolve(
        ...
        Args:
            ...
            num_time_steps (int): number of  for the time evolution.
        ...
        for tt in times[1:]:
            ...
            #ham = hamiltonian
            #print('below is the hamiltonian')
            #print(ham)
